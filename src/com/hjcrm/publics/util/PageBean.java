...
public class PageBean implements Serializable {
	...
	public static String jsonToPage(Object data,PageBean pb) {
		if (data != null) {
			return new StringBuffer().append("{\"total\":\"")
					.append(pb != null ? pb.getCountResult() : 0)
					.append("\",\"currentPage\":\"")
					.append(pb != null ? pb.getCurrentPage() : 0)
	...
	/**
	 * 将用户列表和参数数据封装成一个json字符串对象
	 * @param list
	 * @param total
	 * @param currentPage
	 * @return
	 */
	public static String jsonUserInfoToPage(List list, int total, int currentPage) {
		if (list != null) {
			return new StringBuffer().append("{\"total\":\"")
					.append(total)
					.append("\",\"currentPage\":\"")
					.append(currentPage)
					.append("\",\"rows\":").append(JsonUtil.object2json(list))
					.append("}").toString();
		}
		return null;
	}
